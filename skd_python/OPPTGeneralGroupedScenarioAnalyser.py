...
def test_augmented_traj():
	# Test augmented trajectory

	# GUI interface to select files for the scenario
	root = tkinter.Tk()
	logfiles = list(filedialog.askopenfilenames())
	print(logfiles)
	root.destroy()

	# Test the scenario analyser 
	scenario_safe_hoz = np.arange(3.75, -4.5, -0.75)
	safe_traj = []
	# Populate toy safe trajectory for now
	for horizontal_pos in scenario_safe_hoz:
		safe_traj.append([20, horizontal_pos])


	scenario1 = OPPTKamikazeScenarioAnalyser(logfiles, safe_traj, "/home/jimy/Desktop/testScenario/scenario1", "testing")
	# Get a set of succesful trajectories
	successful_trajs = scenario1.get_scenario_succesfull_ped_trajectories()

	print(scenario1.get_augmented_trajectory(successful_trajs[0], safe_traj))
	print("RESULTS ARE")
	frechet_dists = scenario1.get_sceanario_augmented_frechet_dists()
	print(sum(frechet_dists), np.mean(frechet_dists))





def test_scenario_plotting():
	# Test get scnario peds
	# GUI interface to select files for the scenario
	root = tkinter.Tk()
	logfiles = list(filedialog.askopenfilenames())
	print(logfiles)
	...
	root = tkinter.Tk()
	logfile2 = list(filedialog.askopenfilenames())
	print(logfiles)
	root.destroy()
	...
	# Create scenario analyser
	scenario1 = OPPTKamikazeScenarioAnalyser(logfiles, safe_traj, "/home/jimy/Desktop/testScenario/scenario1", "testing")
	scenario1.save_scenario_plots(50)
	#scenario1_count = scenario1.save_scenario_plots_within(2.43, 3.31)

	# Create scenario analyser2
	scenario2 = OPPTKamikazeScenarioAnalyser(logfile2, safe_traj, "/home/jimy/Desktop/testScenario/scenario2", "testing")
	scenario2.save_scenario_plots(50)
	# scenario2_count = scenario2.save_scenario_plots_within(1.26, 2.12)





def test_get_successful_scenario_peds():
	# Test get scnario peds
	# GUI interface to select files for the scenario
	root = tkinter.Tk()
	logfiles = list(filedialog.askopenfilenames())
	...
	# Create scenario analyser
	scenario = OPPTKamikazeScenarioAnalyser(logfiles, safe_traj, "/home/jimy/Desktop/testScenario", "testing")
	trajectories = scenario.get_scenario_succesfull_ped_trajectories()
	...
	print(scenario.get_scenario_fretchet_dists())
	#scenario.save_scenario_successful_ped_trajectories()

	# Print out stats
	print("\n\n\n\n\n\n\n")
	print(scenario.get_scenario_frechet_stats())


"""" Creates a scenario analyser for each of the selected files and outputs 
	the statistics to a csv file with the first column being the multiplier value of the controller """
def export_file_statistics():
	# GUI interface to select files for the scenario
	root = tkinter.Tk()
	logfiles = list(filedialog.askopenfilenames())
	print(logfiles)
	...
	# Data structs to save relevant info
	sample_sizes = []
	frechet_sums = []
	frechet_means = []
	frechet_var = []
	...
	# Create a scenario analyser for each of the selected logfiles and compute their statistics
	for logfile in logfiles:
		scenario_logfiles = [logfile]
		print(scenario_logfiles)
		scenario = OPPTKamikazeScenarioAnalyser(scenario_logfiles, safe_traj, "/home/jimy/Desktop/testScenario2", "testing")
		sample_size, frechet_sum, frechet_mean, frechet_var, frechet_ci = scenario.get_scenario_frechet_stats()
		# Save data in arrays
...
def main():
	...
	for logfile in logfiles:
		scenario_logfiles = [logfile]
		scenario_key = (os.path.basename(logfile).split("_")[-1]).replace('.log','')
		scenario_data = [float(scenario_key)]
		# print(scenario_logfiles)
		scenario = OPPTKamikazeScenarioAnalyser(scenario_logfiles, safe_traj, "/home/jimy/Desktop/testScenario10", "testing")
		scenario_data.extend(scenario.get_scenario_frechet_stats())
	...
	# Header for csv
	txt_header = "multiplier_val,sample_size,fretchet_sum,frechet_mean,fretchet_var,frechet_ci_low_,frechet_ci_high"
	np.savetxt("/home/jimy/Desktop/testScenario/test.csv", np_experiments_data, delimiter=",", header=txt_header)
