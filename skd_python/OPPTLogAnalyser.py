...
################################### EXTRACT INFORMATION FROM RUNS IN LOG FILE ######################################
	...
	def get_successful_ped_veh_trajectories(self):
		...
		for run_number in range(self.batch_size):
			if(self.run_statuses[run_number] == True):
				# Append the successful trajectory to the list
				ped_trajectories.append(self.get_ped_trajectory(run_number))
				veh_trajectories.append(self.get_veh_trajectory(run_number))

		return copy.deepcopy(ped_trajectories), copy.deepcopy(veh_trajectories)




	""" Saves plots of the successful pedestrian trajectories that had fretchet distance withing a certain bound """
	def plot_successful_trajs(self, other_traj, limit=-1):
		# Compute the run numbers of the successful trajectories
		assert (self.processed_file), "Log file has not been processed"
		assert (self.batch_size == len(self.run_statuses)), "Number of processed runs does not match success stats"
		...
		# Set limit of plots
		NUM_SIZE = self.batch_size

		if((limit > 0) and (limit < self.batch_size)):
			NUM_SIZE = limit

		for run_number in range(self.batch_size):
			if(self.run_statuses[run_number] == True):
				# Successful run trajectory
				run_traj = self.get_ped_trajectory(run_number)
				traj_frechet_dist =  Fretchet.frechetDist(run_traj, other_traj)
				self.save_plot_number(run_number, traj_frechet_dist)
				plot_count += 1

			if(plot_count >= NUM_SIZE):
				break


		# Returns the size of the plots save
		return plot_count


	""" Saves plots of the successful pedestrian trajectories that had fretchet distance withing a certain bound """
	def plot_successful_trajs_within(self, other_traj, lower_bound, upper_bound, limit=-1):
		# Compute the run numbers of the successful trajectories
		assert (self.processed_file), "Log file has not been processed"
		assert (self.batch_size == len(self.run_statuses)), "Number of processed runs does not match success stats"
		...
		for run_number in range(self.batch_size):
			if(self.run_statuses[run_number] == True):
				...
				run_traj = self.get_ped_trajectory(run_number)
				traj_frechet_dist =  Fretchet.frechetDist(run_traj, other_traj)
				if((traj_frechet_dist >= lower_bound) and (traj_frechet_dist <= upper_bound)):
					self.save_plot_number(run_number, traj_frechet_dist)
					plot_count += 1
		...
		# Returns the number of plots saved
		return plot_count
...
def test_get_ped_trajectories():
	...
	# Create directory for storing the split files
	output_dir = "/home/jimy/Desktop/tmp_experiments"
	analyser = OPPTLogAnalyser(output_dir, logfile)
	analyser.split_runs()

	# Print all trajectories in the given run
	print(analyser.get_num_runs())
	a = analyser.get_all_ped_trajectories()
	print(len(a))
	for i in a:
		print(i)
...
def test_get_successful_info():
	...
	analyser = OPPTLogAnalyser(output_dir, logfile)
	analyser.split_runs()
	print(analyser.get_run_timings())

	# # Check if processed
	# if(analyser.is_processed()):
	# 	print(analyser.get_success_statistics())

	# # Print the successul trajectories
	# trajs = analyser.get_successful_ped_trajectories()
	# print(len(trajs))
	# input()
	# for i in trajs:
	# 	print(i)
